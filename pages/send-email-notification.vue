...
<template>
    <div class="p-6 bg-gray-50">
        ...
        <div class="flex items-center gap-6 mb-4">
            <h1 class="text-2xl font-semibold text-gray-900">Send Email Notification</h1>
        </div>
                ...
                <div class="bg-white rounded-xl p-5">
                    ...
                    <div class="flex gap-4 mt-2">
                        <div class="flex-1">
                            ...
                            <Tailwinddropdown v-model="propertyFilters.agencyName" button-class="py-2 bg-[#F8F8F8]" placeholder="Select Agency"
        ...
        <DataTable :data="filteredCustomers" :columns="tableColumns" :initial-items-per-page="10"
            ...
            <template #cell-customer="{ item }">
                <div
                    :class="['flex', item.customer.country ? 'items-center space-x-2 justify-start' : 'justify-center']">
                    <img v-if="item.customer.flag" :src="item.customer.flag" :alt="item.customer.country || 'flag'"
                        class="w-5 h-5" />
                    <span v-if="item.customer.country" class="text-sm font-medium text-black"> {{ item.customer.country
                        }} </span>
                </div>
            </template>
